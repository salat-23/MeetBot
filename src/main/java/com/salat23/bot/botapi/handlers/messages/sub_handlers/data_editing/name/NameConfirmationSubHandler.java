...
@Component
public class NameConfirmationSubHandler implements IMessageSubHandler {

    private final IConfirmationUtility confirmationUtility;
    private final MessageBuilder messageBuilder;
    private final UserRepository userRepository;

    public NameConfirmationSubHandler(IConfirmationUtility confirmationUtility, MessageBuilder messageBuilder, UserRepository userRepository) {
    ...
    public List<UserState> getSupportedStates() {
        return List.of(UserState.CONFIRMING_NAME);
    }
    ...
    public boolean handle(User user, String message) throws TelegramApiException {
        ...
        if (isPositive) {
            ...
            String responseText = messageBuilder.getMessageTextByType(ResponseTemplateTypes.NAME_ENTER_AFTER_CONFIRM);
            ...
            String responseText = messageBuilder.getMessageTextByType(ResponseTemplateTypes.NAME_ENTER_AFTER_DECLINE);
            responseText = messageBuilder.buildAssembleText(new MessageContext(user), responseText);
            Bot.getInstance().execute(messageBuilder.createMessage(user, responseText));
        }
        return true;

...
@Component
public class TargetAgeMinAskSubHandler implements IMessageSubHandler {

    private final MessageBuilder messageBuilder;
    private final UserRepository userRepository;

    public TargetAgeMinAskSubHandler(MessageBuilder messageBuilder, UserRepository userRepository) {
    ...
    public List<UserState> getSupportedStates() {
        return List.of(UserState.ASK_TARGET_AGE_MIN);
    }

    @Override
    public boolean handle(User user, String message) throws TelegramApiException {
        user.setState(UserState.ENTERING_TARGET_AGE_MIN);
        userRepository.save(user);

        String response = messageBuilder
                .getMessageTextByType(ResponseTemplateTypes.TARGET_AGE_MIN_ASK, user.getGender());
        ...
        if (user.getTargetMinAge() != null)
            sendMessage.setReplyMarkup(messageBuilder.getPreviousOptionKeyboard(user.getTargetMinAge().toString()));

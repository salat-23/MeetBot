...
@Component
public class PhotoConfirmationSubHandler implements IMessageSubHandler {

    private final IConfirmationUtility confirmationUtility;
    private final MessageBuilder messageBuilder;
    private final UserRepository userRepository;

    public PhotoConfirmationSubHandler(IConfirmationUtility confirmationUtility, MessageBuilder messageBuilder, UserRepository userRepository) {
    ...
    public boolean handle(User user, String message) throws TelegramApiException {
        ...
        if (isPositive) {
            ...
            String responseText = messageBuilder.getMessageTextByType(ResponseTemplateTypes.PHOTO_ENTER_AFTER_CONFIRM);
            ...
            String responseText = messageBuilder.getMessageTextByType(ResponseTemplateTypes.PHOTO_ENTER_AFTER_DECLINE);
            responseText = messageBuilder.buildAssembleText(new MessageContext(user), responseText);
            Bot.getInstance().execute(messageBuilder.createMessage(user, responseText));
        }
        return true;

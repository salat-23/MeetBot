...
@Component
public class TargetAgeAskConfirmationSubHandler implements IMessageSubHandler {

    private final MessageBuilder messageBuilder;
    private final UserRepository userRepository;

    public TargetAgeAskConfirmationSubHandler(MessageBuilder messageBuilder, UserRepository userRepository) {
    ...
    public List<UserState> getSupportedStates() {
        return List.of(UserState.ASK_CONFIRMING_TARGET_AGE);
    }

    @Override
    public boolean handle(User user, String message) throws TelegramApiException {

        user.setState(UserState.CONFIRMING_TARGET_AGE);
        userRepository.save(user);

        //Send the message asking user to confirm its target age
        String confirmMessage = messageBuilder.getMessageTextByType(ResponseTemplateTypes.TARGET_AGE_CONFIRM,
                user.getGender());

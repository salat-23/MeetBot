...
@Component
public class NameAskConfirmationSubHandler implements IMessageSubHandler {

    private final MessageBuilder messageBuilder;
    private final UserRepository userRepository;

    public NameAskConfirmationSubHandler(MessageBuilder messageBuilder, UserRepository userRepository) {
    ...
    public List<UserState> getSupportedStates() {
        return List.of(UserState.ASK_CONFIRMING_NAME);
    }

    @Override
    public boolean handle(User user, String message) throws TelegramApiException {

        user.setState(UserState.CONFIRMING_NAME);
        userRepository.save(user);

        //Create confirmation message text and send it
        String confirmMessage = messageBuilder.getMessageTextByType(ResponseTemplateTypes.NAME_ENTER_CONFIRM);

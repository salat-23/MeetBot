...
@Component
public class GenderConfirmationSubHandler implements IMessageSubHandler {

    private final IConfirmationUtility confirmationUtility;
    private final MessageBuilder messageBuilder;
    private final UserRepository userRepository;

    public GenderConfirmationSubHandler(IConfirmationUtility confirmationUtility, MessageBuilder messageBuilder, UserRepository userRepository) {
    ...
    public List<UserState> getSupportedStates() {
        return List.of(UserState.CONFIRMING_GENDER);
    }
    ...
    public boolean handle(User user, String message) throws TelegramApiException {
        ...
        if (isAccept) {
            ...
            String postConfirmMessage = messageBuilder
                    .getMessageTextByType(ResponseTemplateTypes.GENDER_ENTER_AFTER_CONFIRM, user.getGender());
        ...
        //Create and send the message
        String postConfirmMessage = messageBuilder
                .getMessageTextByType(ResponseTemplateTypes.GENDER_ENTER_AFTER_DECLINE);
        postConfirmMessage = messageBuilder.buildAssembleText(new MessageContext(user), postConfirmMessage);
        Bot.getInstance().execute(messageBuilder.createMessage(user, postConfirmMessage));
